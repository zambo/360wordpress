...
** - restore PNG version
** - delete PNG backup when JPG deleted
*/
	...
	function tab_convert(){
		...
		<div class="below-h2 error">
			<p>
				...
				<em><?php echo "{$wpdb->prefix}posts, {$wpdb->prefix}postmeta, {$wpdb->prefix}options, {$wpdb->prefix}yoast_seo_links, {$wpdb->prefix}revslider_slides, {$wpdb->prefix}revslider_static_slides, {$wpdb->prefix}toolset_post_guid_id, {$wpdb->prefix}fpd_products, {$wpdb->prefix}fpd_views, {$wpdb->prefix}blc_instances, {$wpdb->prefix}blc_links" ?></em>. 
		...
		foreach( $replaces as $old => $new ){
			...
			if( in_array( $table_name, $this->db_tables ) ){
				$wpdb->query("
					UPDATE $table_name 
					SET thumbnail = REPLACE( thumbnail, '/{$old}', '/{$new}'), 
						elements = REPLACE( elements, '/{$old}', '/{$new}') 
					WHERE thumbnail LIKE '%/{$old}%' 
						OR elements LIKE '%/{$old}%'
				");
			}
			...
			if( in_array( $table_name, $this->db_tables ) ){
				$wpdb->query("
					...
					SET link_text = REPLACE( link_text, '/{$old}', '/{$new}'), 
						raw_url = REPLACE( raw_url, '/{$old}', '/{$new}') 
			...
			if( in_array( $table_name, $this->db_tables ) ){
				$wpdb->query("
					UPDATE $table_name 
					SET url = REPLACE( url, '/{$old}', '/{$new}'), 
						final_url = REPLACE( final_url, '/{$old}', '/{$new}'), 
						log = REPLACE( log, '/{$old}', '/{$new}'), 
						result_hash = REPLACE( result_hash, '/{$old}', '/{$new}') 
					WHERE url LIKE '%/{$old}%' 
						OR final_url LIKE '%/{$old}%' 
						OR log LIKE '%/{$old}%' 
						OR result_hash LIKE '%/{$old}%'
				");
			}
		}
	}
